...
from dqn_model import Agent, ReplayMemoryPER
...
def evaluate(args, kg, dataset, filename,epoch,difficulty):
    ...
    agent = Agent(device=args.device,env=test_env, memory=memory, state_size=args.hidden, action_size=embed.size(1), \
    ...
    agent.load_model(data_name=args.data_name, filename=filename, epoch_user=epoch,data_type=args.data_type,mastery=args.mastery,difficulty=difficulty)
    ...
    for user_num in tqdm(range(user_size)):  #user_size
        ...
        if (user_num+1) % args.observe_num == 0 and user_num > 0:
            SR = [SR5/args.observe_num,SR10/args.observe_num, SR15/args.observe_num, SR20/args.observe_num, AvgT / args.observe_num, patience / args.observe_num,delay_step/ args.observe_num]
...
def main():
    ...
    parser.add_argument('--mastery', type=str, default='math-dqn-best', help='the type of data')
    ...
    for epoch,difficulty in [(3,'0.5')]:
        evaluate(args, kg, dataset, filename,epoch,difficulty)
