...
class BinaryRecommendEnv(object):
    def __init__(self, kg, dataset, data_name, embed,data_type, master_level,seed=1, max_turn=15, cand_num=10, cand_item_num=10, attr_num=20, mode='train', ask_num=1, entropy_way='weight entropy', fm_epoch=0):
        ...
        self.ui_dict = self.__load_rl_data__mooc(data_name, data_type,master_level,mode=mode)  # np.array [ u i weight]
    ...
    def __load_rl_data__mooc(self, data_name,data_type,master_level,mode):
        if mode == 'train':
           with open(os.path.join(DATA_DIR[data_name], 'UI_Interaction_data'+TYPE_DIR[data_type]+'/train_dict.json'), encoding='utf-8') as f:
            ...
            with open(os.path.join(DATA_DIR[data_name],'UI_Interaction_data'+TYPE_DIR[data_type]+'/test_dict'+master_level+'.json'), encoding='utf-8') as f:
                print('test_data: load RL test data')
                mydict = json.load(f)
        return mydict
    ...
    def reset(self, embed=None):
        ...
        self.candidate_concept =  self._update_cand_concepts()
        ...
        print('predecessor num: {}'.format(len(self.candidate_concept)-1))

        return self._get_state(), self._get_cand(), self._get_action_space()

    def _get_cand(self):
        target_feature = self._map_to_all_id([self.target_item],'feature')
        self.cand_items_thisround=self.select_cand_concepts()
        # a=self.test_ex(self.net,self.user_id,self.cand_items_thisround)
        if self.random_sample_item:
            ...
            cand_item = self._map_to_all_id(self.cand_items_thisround[:self.cand_item_num],'item')
        if self.ask_open:
    ...
    def _get_state(self):
        ...
        for item in self_cand_items:
            ...
            for fea in self.item_feature_pair[item]:
                ...
                v.append(1)
        target_idx=self.target_item+self.user_length+self.item_length
        for concept in self.candidate_concept:
            concept_idx=concept+self.user_length+self.item_length
            i.append([idx[target_idx],idx[concept_idx]])
            i.append([idx[concept_idx],idx[target_idx]])
            v.append(1)
            v.append(1)
        user_idx = idx[self.user_id]
        
        for item, score in self.exer_score.items():
            item_idx = item + self.user_length
    ...
    def step(self, action, sorted_actions, embed=None):  
        ...
        cand_items = [item for values in self.pre_list.values() for item in values]
        ...
        if self.cur_conver_step == self.max_turn:
            ...
            self.conver_his[self.cur_conver_step-1] = self.history_dict['cand_none']
            print('--> no cand items!')
            done = 1
        elif action >= self.user_length + self.item_length:   #ask feature
            asked_feature = self._map_to_old_id(action)
            self.action_type='concept'
            self.action_id=asked_feature
            print('test concept')
            ...
            self.action_type='exercise'
            self.action_id=action
            reward, done = self._recommend_update(action)  
               
        self.cand_items=list(set([item for values in self.pre_list.values() for item in values]))
        if self.cand_items==[]:
            self.action_type='done'
            self.action_id='done'
            reward = self.reward_dict['cand_none']
    ...
    def _update_cand_concepts(self):
        ...
        self.cand_items=concept_exercise
        self.all_exercises=concept_exercise
        prede=list(self.kg.G['concept'][self.target_item]['predecessor'])
        prede_list={}
        out_put_={}
        prede_list[self.target_item]=set(concept_exercise)
        for i in prede:
            prede_list[i]=set(concept_exercise).intersection(set(self.kg.G['concept'][i]['belong_to']))
            prede_list[self.target_item]=prede_list[self.target_item].difference(set(self.kg.G['concept'][i]['belong_to']))                
        filtered_dict = {key: value for key, value in prede_list.items() if len(value) != 0}
        # for i,exercises in filtered_dict.items():
        #         out_put_[i]=self.test_master(net,self.user_id,exercises)
        # sorted_keys = sorted(filtered_dict, key=lambda x: out_put_[x],reverse=True)
        # sorted_filtered_dict = {key: filtered_dict[key] for key in sorted_keys}
        # if self.target_item in sorted_keys:
        #     sorted_keys.remove(self.target_item)
        #     sorted_keys.append(self.target_item)
        #去除字典中重复出现的exercise
        # current_exercise=[]
        # for i,exercise in sorted_filtered_dict.items():
        #     repeat_exercise=set(current_exercise).intersection(exercise)
        #     if repeat_exercise:
        #         sorted_filtered_dict[i]=exercise-repeat_exercise
        #     current_exercise.extend(exercise)
        #     current_exercise=list(set(current_exercise))
        # filtered_dict = {key: value for key, value in sorted_filtered_dict.items() if len(value) != 0}
        # reversed_dict = {value: key for key, value in filtered_dict.items()}
        self.pre_list=filtered_dict
        candidate_concepts=list(filtered_dict.keys())
    ...
    def _ask_update(self, target_item):
        ...
        self.action_score=current_lev.item()
    ...
    def _item_score(self):
        ...
        for item_id in self.cand_items:
            ...
            unprefer_item = list(self.user_rej_item)
    ...
    def select_cand_concepts(self):
        ...
        sorted_pre_list = {key: list(self.pre_list[key]) for key in sorted_keys}
        for key,value in sorted_pre_list.items():
            sorted_list = sorted(value, key=lambda x: self.cand_item_score.get(x, 0), reverse=True)
            sorted_pre_list[key] = sorted_list
    ...
    def _recommend_update(self, recom_items):
        ...
        if self.difficulty_record>=self.patience:
            ...
            self.exer_score[recom_items]=-1
            # for i,exercises in self.pre_list.items():
            #     if recom_items in exercises:
            #         exercises.remove(recom_items)
            ...
            self.user_acc_item.append(recom_items)
            reward = self.reward_dict['ask_suc'] 
            self.conver_his[self.cur_conver_step] = self.history_dict['ask_suc'] #update state vector: conver_his
            for i,exercises in self.pre_list.items():
                if recom_items in exercises:
                    exercises.remove(recom_items)
            self.exer_score[recom_items]=1
    ...
    def load_snapshot(self, model,filename):
        ...
        f.close()
    def test_concept(self,net,user,concept,exercises=[]):
        output=[]
        device=self.device
        if len(exercises)==0:
            exercises=self.kg.G['concept'][concept]['belong_to']
    ...
    def restore_cand_items(self):
        ...
        for i in user_rej_item:
            for pre in self.candidate_concept:
                if pre in self.kg.G['exercise'][i]['belong_to']:
                    self.pre_list[pre].add(i)
