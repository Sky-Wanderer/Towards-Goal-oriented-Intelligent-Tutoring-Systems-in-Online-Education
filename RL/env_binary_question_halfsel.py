...
class BinaryRecommendEnv(object):
    ...
    def _get_cand(self):
        self.cand_this=self.select_cand_concepts()
        target_feature = self._map_to_all_id([self.target_item],'feature')
        cand_item = self._map_to_all_id(self.cand_this,'item')
        if self.ask_open:
    ...
    def _get_action_space(self):
        if self.ask_open:
            action_space = [self._map_to_all_id([self.target_item],'feature'),self._map_to_all_id(self.cand_this,'item')]
        else:
            action_space = [self._map_to_all_id(self.cand_this,'item')]
        return action_space
    ...
    def select_cand_concepts(self):
        ...
        sorted_items =  sorted(self.cand_items, key=lambda x: self.cand_item_score.get(x, 0), reverse=True)
