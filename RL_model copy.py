...
#TODO select env
from RL.env_binary_question_noselection import BinaryRecommendEnv
...
def train(args, kg, dataset, filename):
    ...
    for difficult in ['4','8','12','16','20']:
        ...
        agent = Agent(device=args.device, memory=memory, state_size=args.hidden, action_size=embed.size(1), \
            hidden_size=args.hidden, gcn_net=gcn_net, learning_rate=args.learning_rate, l2_norm=args.l2_norm, PADDING_ID=embed.size(0)-1)
        env.setmastery__patiency(0.5,difficult)
        for train_step in range(1, args.max_steps+1):
            ...
            if SR20>SR20_record:
                SR20_record=SR20
                agent.save_model(data_name=args.data_name, filename=filename, epoch_user=train_step,data_type=args.data_type,mastery='com-difficult',difficulty=difficult)
