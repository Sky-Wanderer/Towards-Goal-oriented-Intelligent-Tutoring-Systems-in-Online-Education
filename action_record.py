...
#TODO select env
from RL.nosel_action import BinaryRecommendEnv
...
def pai_evaluate(args, kg, dataset, filename,epoch,difficulty,masterlevel):
    test_env = EnvDict[args.data_name](kg, dataset, args.data_name, args.embed,args.data_type, master_level=masterlevel,seed=args.seed, max_turn=args.max_turn,
    ...
    with open(os.path.join('./id_entity.json'), encoding='utf-8') as f:
    ...
    for user_num in tqdm(range(user_size)):  #user_size
        ...
        is_last_turn = False
        test_env.setmastery__patiency(0.5,difficulty)
        ...
        for t in count():  # user  dialog
            ...
            if test_env.action_type=='concept':
                action_record.append(test_env.action_type+'_'+str(id_entity['concept'][str(test_env.action_id)])+':'+str(test_env.action_score))
            elif test_env.action_type=='exercise' :
                action_record.append(test_env.action_type+'_'+str(id_entity['exercise'][str(test_env.action_id)])+':'+str(test_env.action_score))
            else:
                action_record.append(test_env.action_type+'_'+str(test_env.action_id)+':'+str(test_env.action_score))
            if t == 19:
                done=1
            if done:
                next_state = None
            ...
            if done:
                ...
                if reward.item() >0.01:  # recommend successfully
                    ...
                    AvgT += 20
                break
        action_record_all.append([id_entity['user'][str(test_env.user_id)],action_record])
    ...
    #     f.write('================================\n')
    file_path = 'action_list_dqn.json'
    # Write the list to a JSON file
    with open(file_path, 'w') as json_file:  
        json.dump(action_record_all, json_file)
    return SRturn_all,SR20_mean,AvgT_mean,patience_mean,user_size
...
def main():
    ...
    parser.add_argument('--mastery', type=str, default='com-dqn-new', help='the type of data')
    ...
    for epoch,difficulty in [(10,'4')]:
        ...
        user_size=[]
        for masterlevel in ['']:
            SRturn_all_,SR20_mean_,AvgT_mean_,patience_mean_,user_size_=pai_evaluate(args, kg, dataset, filename,epoch,difficulty,masterlevel)
            SRturn_all.append(SRturn_all_)
            SR20_mean.append(SR20_mean_)
            AvgT_mean.append(AvgT_mean_)
            patience_mean.append(patience_mean_)
            user_size.append(user_size_)
        
        enablePrint()
        print('SRturn',SRturn_all)
        print('sr20',SR20_mean)
        print('avg_turn',AvgT_mean)
        print('patience',patience_mean)
        print('user_size',user_size)
        
        easy_weighted_turn = [x * user_size[0] for x in SRturn_all[0]]
        mid_weighted_turn = [x * user_size[1] for x in SRturn_all[1]]
        dif_weighted_turn = [x * user_size[2] for x in SRturn_all[2]]
        total_num=sum(user_size)
        total_weighted_turn = [sum(x) for x in zip(easy_weighted_turn, mid_weighted_turn, dif_weighted_turn)]
        mean_weighted_turn = [x/total_num for x in total_weighted_turn]
        print('3 turn avg:',mean_weighted_turn)
        
        avg_all=(AvgT_mean[0]*user_size[0]+AvgT_mean[1]*user_size[1]+AvgT_mean[2]*user_size[2])/total_num
        patience_all=(patience_mean[0]*user_size[0]+patience_mean[1]*user_size[1]+patience_mean[2]*user_size[2])/total_num
        sr20_all=(SR20_mean[0]*user_size[0]+SR20_mean[1]*user_size[1]+SR20_mean[2]*user_size[2])/total_num
        print('avg turn:',avg_all)
        print('patience_all:',patience_all)
        print('sr20:',sr20_all)
