...
#TODO select env
from RL.dqn import BinaryRecommendEnv
from RL.RL_evaluate import dqn_evaluate
from RL.dqn_evaluate import evaluate
...
class Agent(object):
    def __init__(self, device,env, memory, state_size, action_size, hidden_size, gcn_net, learning_rate, l2_norm, PADDING_ID, EPS_START = 0.9, EPS_END = 0.1, EPS_DECAY = 0.0001, tau=0.01):
        ...
        self.tau = tau
        self.ui_emb=env.ui_embeds
        self.feature_emb=env.feature_emb
    ...
    def select_action(self, state, cand, action_space, is_test=False, is_last_turn=False):
        ...
        if is_test or sample > eps_threshold:
            # if is_test and (len(cand[0]) <= 1 or is_last_turn):
            #     return torch.tensor(action_space[0][0], device=self.device, dtype=torch.long), cand
            with torch.no_grad():
            ...
            if possibility<0.5:
                return torch.tensor(action_space[0][0], device=self.device, dtype=torch.long), shuffled_cand
    ...
    def optimize_model(self, BATCH_SIZE, GAMMA):
        ...
        tensor_values = [tensor.item() for tensor in batch.action]
        # Create a new tensor with the extracted values and reshape it to (128, )
        combined_tensor = torch.tensor(tensor_values, device='cpu').view(128,1)
        # Move the combined tensor to your desired device
        action_batch = combined_tensor.to(self.device)
        
        action_emb_batch = self.gcn_net.embedding(action_batch)
        
        
        tensor_values = [tensor.item() for tensor in batch.reward]
        # Create a new tensor with the extracted values and reshape it to (128, )
        combined_tensor = torch.tensor(tensor_values, device='cpu').view(128,1)
        # Move the combined tensor to your desired device
        reward_batch = combined_tensor.to(self.device)
    ...
    def save_model(self, data_name, filename, epoch_user,data_type,mastery,difficulty):
        save_rl_agent(dataset=data_name, model={'policy': self.policy_net.state_dict(), 'gcn': self.gcn_net.state_dict()}, filename=filename, epoch_user=epoch_user,data_type=data_type,mastery=mastery,difficulty=difficulty)
    def load_model(self, data_name, filename, epoch_user,data_type,mastery,difficulty):
        model_dict = load_rl_agent(dataset=data_name, filename=filename, epoch_user=epoch_user,data_type=data_type,mastery=mastery,difficulty=difficulty)
...
def train(args, kg, dataset, filename):
    env = EnvDict[args.data_name](kg, dataset, args.data_name, args.embed,args.data_type, seed=args.seed, max_turn=args.max_turn, cand_num=args.cand_num, cand_item_num=args.cand_item_num,
    ...
    gcn_net = GraphEncoder(device=args.device, entity=embed.size(0), emb_size=embed.size(1), kg=kg, embeddings=embed, \
        fix_emb=args.fix_emb, seq=args.seq, gcn=False, hidden_size=args.hidden).to(args.device)
    agent = Agent(device=args.device, env=env, memory=memory, state_size=args.hidden, action_size=embed.size(1), \
    ...
    for demand in ['0.5']:
        ...
        agent = Agent(device=args.device,env=env,  memory=memory, state_size=args.hidden, action_size=embed.size(1), \
            hidden_size=args.hidden, gcn_net=gcn_net, learning_rate=args.learning_rate, l2_norm=args.l2_norm, PADDING_ID=embed.size(0)-1)
        env.setmastery__patiency(demand,4)
        for train_step in range(1, args.max_steps+1):
            SR10, SR15, SR20, AvgT, PATIENCE,total_reward = 0., 0., 0., 0., 0., 0.
            ...
            for i_episode in tqdm(range(args.sample_times),desc='sampling'):
                ...
                for t in count():   # user  dialog
                    ...
                    if done:
                        ...
                        AvgT += t+1
                        PATIENCE+=env.difficulty_record
                        total_reward += env.cur_conver_step-env.mastery_step
                        break
            enablePrint() # Enable print function
            print('loss : {} in epoch_uesr {}'.format(loss.item()/args.sample_times, args.sample_times))
            print('SR10:{}, SR15:{}, SR20:{}, AvgT:{}, PATIENCE:{}, rewards:{} '
                    'Total epoch_uesr:{}'.format(SR10 / args.sample_times, SR15 / args.sample_times, SR20 / args.sample_times,
                                                    AvgT / args.sample_times, PATIENCE/ args.sample_times, total_reward / args.sample_times, args.sample_times))

            # if train_step % args.eval_num == 0:
            #     SR15_mean = dqn_evaluate(args, kg, dataset, agent, filename, train_step)
            #     test_performance.append(SR15_mean)
            
            SR15_mean=evaluate(args, kg, dataset, filename,agent,demand)
            if SR15_mean>SR15_record:
                print('save model!',SR15_mean)
                SR15_record=SR15_mean
                agent.save_model(data_name=args.data_name, filename=filename, epoch_user=train_step,data_type=args.data_type,mastery='com-dqn-best',difficulty=demand)
...
def main():
    ...
    parser.add_argument('--data_name', type=str, default=MOOC, choices=[LAST_FM, LAST_FM_STAR, YELP, YELP_STAR, MOOC],
                        help='One of {LAST_FM, LAST_FM_STAR, YELP, YELP_STAR, MOOC}.')
    ...
    parser.add_argument('--max_turn', type=int, default=21, help='max conversation turn')
    ...
    parser.add_argument('--observe_num', type=int, default=5, help='the number of steps to print metric')
    ...
    parser.add_argument('--data_type', type=str, default='COM', help='the type of data')
    parser.add_argument('--mastery', type=str, default='0.7', help='the type of data')
    parser.add_argument('--difficulty', type=str, default='3.5', help='the type of data')
    ...
    print('dataset:{}, concept_length:{}'.format(args.data_name, feature_length))
    args.attr_num = feature_length  # set attr_num  = feature_length
    print('args.concept_num:', args.attr_num)
