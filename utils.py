...
def load_dataset(dataset,data_type):
    dataset_file = TMP_DIR[dataset]  + TYPE_DIR[data_type] + '/dataset.pkl'
...
def load_kg(dataset,data_type):
    kg_file = TMP_DIR[dataset] + TYPE_DIR[data_type]+ '/kg.pkl'
...
def load_embed(dataset, embed, data_type):
    if embed:
        path = TMP_DIR[dataset] + '/embeds' + TYPE_DIR[data_type]+ '/{}.ckpt'.format(embed)
    else:
        return None
...
def load_rl_agent(dataset, filename, epoch_user,data_type,mastery,difficulty):
    model_file = TMP_DIR[dataset] + '/RL-agent'+ '/'+ filename +'-'+data_type+'-'+mastery+'-'+difficulty+ '-epoch-{}.pkl'.format(epoch_user)
    model_dict = torch.load(model_file)
    print('RL policy model load at {}'.format(model_file))
    return model_dict

def save_rl_agent(dataset, model, filename, epoch_user,data_type,mastery,difficulty):
    model_file = TMP_DIR[dataset] + '/RL-agent/' + filename +'-'+data_type+'-'+mastery+'-'+difficulty+ '-epoch-{}.pkl'.format(epoch_user)
