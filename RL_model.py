...
#TODO select env
from RL.env_binary_question_halfsel import BinaryRecommendEnv
...
import time
import warnings
from RL.pai_evaluate import pai_evaluate
...
def train(args, kg, dataset, filename):
    env = EnvDict[args.data_name](kg, dataset, args.data_name, args.embed,args.data_type,  master_level='',seed=args.seed, max_turn=args.max_turn, cand_num=args.cand_num, cand_item_num=args.cand_item_num,
   ...
   # self.reward_dict = {
    ...
    for difficulty in ['0.5']:
        ...
        # agent.load_model(data_name=args.data_name, filename=filename, epoch_user=6,data_type=args.data_type,mastery='com-best2',difficulty='0.5')

       
        env.setmastery__patiency(difficulty,4)
        for train_step in range(1, args.max_steps+1):
            ...
            #     test_performance.append(SR15_mean)
            SR15_mean = pai_evaluate(args, kg, dataset, filename,agent,  difficulty)
            
            if SR15_mean>SR15_record:
                print('save model!',SR15_mean)
                SR15_record=SR15_mean
                agent.save_model(data_name=args.data_name, filename=filename, epoch_user=train_step,data_type=args.data_type,mastery='com-halfsel',difficulty=difficulty)
...
def main():
    ...
    parser.add_argument('--fm_epoch', type=int, default=1, help='the epoch of FM embedding')
    ...
    parser.add_argument('--max_steps', type=int, default=21, help='max training steps')
    ...
    parser.add_argument('--mastery', type=str, default='0.5', help='the type of data')
